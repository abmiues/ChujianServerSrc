...
<script type="text/javascript">
						...
						$
								.ajax({
									...
									success : function(datas) {
										...
										$("#cameralist").empty();
										var htmlstr = "";
										htmlstr += "<div class='container-fluid' style='background:#ffffff'><div class='col-md-2'></div><div class='col-md-2'>名称</div><div class='col-md-2'>状态</div><div class='col-md-6'>地址</div></div>";
										for (var i = 0; i < datas.length; i++) {
											...
											if (i % 2 == 0)
												htmlstr += "<div class='container-fluid' style='background:#fafafa;height:50px;line-height:50px'><div class='col-md-2'><img src='images/"+<%=userid%>+"/camera"+datas[i].cameraid+".png' style='height: 40px; margin-right: 10px;margin-top: 5px;'></img></div><div class='col-md-2'><a href='javascript:showdetail(\""
														+ i
														+ "\")'>"
														+ datas[i].camname
														+ "</a></div><div class='col-md-2'>"
														+ camerastate[datas[i].state]
														+ "</div><div class='col-md-6'>"
														+ datas[i].address
														+ "</div></div>";
											else
												htmlstr += "<div class='container-fluid' style='background:#ffffff;height:50px;line-height:50px'><div class='col-md-2'><img src='images/"+<%=userid%>+"/camera"+datas[i].cameraid+".png' style='height: 40px; margin-right: 10px;margin-top: 5px;'></img></div><div class='col-md-2'><a href='javascript:showdetail(\""
												+ i
												+ "\")'>"
												+ datas[i].camname
												+ "</a></div><div class='col-md-2'>"
	...
	function showdetail(j) {
		...
		//"cameraimg"
		$("#fooddetailimg").attr("src","images/"+parentdata.sellerid+"/camera"+parentdata.cameraid+".png");
		$("#div_detail").empty();
		...
		$.ajax(
			  	{
			  		 cache: true,
			                type: "POST",
			                url:"seller/addcamera",
			                data:$("#camerainfo").serialize(),// 你的formid
			                async: false,
			                error: function(request) {
			                    alert("服务器连接失败");
			                },
		...
		if (value != "") {
			$.ajaxFileUpload({
				...
				success : function(data, status) {
					if (data == '111')
						window.location.href="index.jsp?id=3";
				},
...
<br/>
	<button type="button" class="btn btn-default" data-toggle="modal"
		data-target="#myModal">添加摄像头</button>
	<!-- Modal -->
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel"
						style="text-align: center">添加摄像头</h4>
				</div>
				<div class="modal-body">
					<div class="container-fluid">
						 <div class='col-md-4' >
							<img src='images/1.png' id="cameraimg" style='height: 100px;'>
						</div>
						<div class='col-md-8'>
					...
					<br/>
					<div class="container-fluid">
						<form action="#" id="camerainfo" method="post"
							enctype="multipart/form-data">
							<span>摄像头名：</span><input name="camname">
							<br/>
							<br/>
							<span>视频地址：</span><input type="text"  name="address" style="width:400px;">
						</form>
					</div>
				</div>
	...
	<div class="container-fluid" id="cameralist"></div>
